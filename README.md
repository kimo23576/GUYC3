...
<head>
  ...
  <title>الإستمارة الذكية لتسجيل الأضرار والخسائر والتعويضات لقطاع المقاولات اليمني</title>
  <!-- استدعاء Font Awesome والخطوط -->
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
  <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700&family=Noto+Naskh+Arabic:wght@400;600;700&display=swap" rel="stylesheet">
  <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"></script>
  <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
  <style>
    /* إعدادات الألوان والخطوط الفخمة */
    :root {
      --primary-color: #3b2f2f;
      --secondary-color: #5e4b4b;
    ...
    .bubble {
      ...
      opacity: 0.2;
      border-radius: 50%;
      animation: rise var(--animation-speed) infinite ease-in;
    }
    @keyframes rise {
      0% {
    ...
    /* إنشاء مجموعة من الفقاعات بحجوم ومواقع مختلفة */
    .bubbles .bubble:nth-child(1) { width: 40px; height: 40px; left: 10%; animation-duration: 8s; animation-delay: 0s; }
    .bubbles .bubble:nth-child(2) { width: 60px; height: 60px; left: 25%; animation-duration: 10s; animation-delay: 2s; }
    .bubbles .bubble:nth-child(3) { width: 30px; height: 30px; left: 40%; animation-duration: 7s; animation-delay: 4s; }
    .bubbles .bubble:nth-child(4) { width: 50px; height: 50px; left: 60%; animation-duration: 9s; animation-delay: 1s; }
    .bubbles .bubble:nth-child(5) { width: 70px; height: 70px; left: 80%; animation-duration: 12s; animation-delay: 3s; }
    ...
    .header h2 { font-size: 1.6rem; margin-bottom: 1rem; }
    .header a { color: #FFD700; text-decoration: none; font-weight: bold; }
    .reference-number { background: var(--accent-color); padding: 1rem 2rem; border-radius: 25px; margin-top: 1.5rem; font-weight: bold; }

    /* تنسيق الأقسام الفاخرة */
    .section {
      background: var(--section-bg);
      border-radius: 15px;
      padding: 2.5rem;
      margin-bottom: 2rem;
      border-left: 6px double var(--section-border);
      position: relative;
      overflow: hidden;
    ...
    .add-btn:hover, .btn:hover { background: var(--secondary-color); }
    .remove-btn {
      background: var(--accent-color);
      color: #fff;
      border: none;
    ...
    .legal-references ul { list-style: none; }
...
<body>
  ...
  <div class="container">
    ...
    <div class="section">
      ...
      <div class="section-intro">
        <p>
          يُعد هذا النظام منصة شاملة لتوثيق وحساب كافة أنواع الأضرار والخسائر الناتجة عن الأزمات، الحروب والنزاعات المسلحة وفقًا للمعايير الدولية وأفضل الممارسات العالمية. يساعد النظام في إعداد ملف مطالبة متكامل مدعومًا بالأدلة والمرجعيات القانونية.
        </p>
        <ul>
          <li><strong>الفئات المستهدفة:</strong> المقاولون، الشركات الإنشائية، الجهات الحكومية، ومنظمات المجتمع المدني.</li>
          <li><strong>أهمية النظام:</strong> توثيق شامل وحساب دقيق لإعداد ملف مطالبة متكامل.</li>
          <li><strong>تنويه:</strong> البيانات للتقييم الأولي وقد تخضع للمراجعة.</li>
        </ul>
      </div>
    </div>
    ...
    <div class="section" id="sectionDirect">
      ...
      <div class="section-intro">
        <p>
          <strong>طريقة الحساب:</strong> تُحسب قيمة الضرر لكل بند بضرب القيمة بنسبة الضرر ومدة الضرر.<br>
      ...
      <div class="dynamic-list" id="fixedAssets">
        <div class="item-card">
          ...
          <button class="remove-btn" onclick="this.parentElement.remove(); calcFixedAssets()"><i class="fas fa-times"></i> حذف</button>
        </div>
      </div>
      ...
      <div class="dynamic-list" id="inventoryDamage">
        <div class="item-card">
          ...
          <button class="remove-btn" onclick="this.parentElement.remove(); calcInventoryDamage()"><i class="fas fa-times"></i> حذف</button>
        </div>
      </div>
      ...
      <div class="dynamic-list" id="financialDirect">
        <div class="item-card">
          ...
          <button class="remove-btn" onclick="this.parentElement.remove(); calcFinancialDirect()"><i class="fas fa-times"></i> حذف</button>
        </div>
      </div>
      ...
      <div class="dynamic-list" id="institutionalDamage">
        <div class="item-card">
          ...
          <button class="remove-btn" onclick="this.parentElement.remove(); calcInstitutionalDamage()"><i class="fas fa-times"></i> حذف</button>
        </div>
      </div>
    ...
    <div class="section" id="sectionIndirect">
      ...
      <div class="section-intro">
        <p>
          <strong>طريقة الحساب:</strong> تُحسب قيمة الضرر غير المباشر بضرب القيمة بنسبة التأثير ومدة الضرر.<br>
      ...
      <div class="dynamic-list" id="productionDelay">
        <div class="item-card">
          ...
          <button class="remove-btn" onclick="this.parentElement.remove(); calcProductionDelay()"><i class="fas fa-times"></i> حذف</button>
        </div>
      </div>
      ...
      <div class="dynamic-list" id="competitiveLoss">
        <div class="item-card">
          ...
          <button class="remove-btn" onclick="this.parentElement.remove(); calcCompetitiveLoss()"><i class="fas fa-times"></i> حذف</button>
        </div>
      </div>
      ...
      <div class="dynamic-list" id="techDamage">
        <div class="item-card">
          ...
          <button class="remove-btn" onclick="this.parentElement.remove(); calcTechDamage()"><i class="fas fa-times"></i> حذف</button>
        </div>
      </div>
      ...
      <div class="dynamic-list" id="envDamage">
        <div class="item-card">
          ...
          <button class="remove-btn" onclick="this.parentElement.remove(); calcEnvDamage()"><i class="fas fa-times"></i> حذف</button>
        </div>
      </div>
    ...
    <div class="section" id="sectionDelayed">
      ...
      <div class="section-intro">
        <p>
          <strong>طريقة الحساب:</strong> يُحسب الضرر بضرب المبلغ المتأخر في مدة التأخير (بالأشهر) مع احتساب فائدة تراكمية بنسبة 5% سنويًا.<br>
      ...
      <div class="dynamic-list" id="delayedPayments">
        <div class="item-card">
          ...
          <button class="remove-btn" onclick="this.parentElement.remove(); calculateSectionTotal('delayedPayments','delayedTotalYER','delayedTotalUSD')"><i class="fas fa-times"></i> حذف</button>
        </div>
      </div>
    ...
    <div class="section" id="sectionOther">
      ...
      <div class="section-intro">
        <p>
          أدخل بيانات أضرار أخرى لم تُدرج في الأقسام السابقة مع تحديد الوصف، القيمة، مدة الضرر، ونسبة التأثير.
        </p>
      </div>
      <div class="dynamic-list" id="otherDamages">
        <div class="item-card">
          ...
          <button class="remove-btn" onclick="this.parentElement.remove(); calculateSectionTotal('otherDamages','otherTotalYER','otherTotalUSD')"><i class="fas fa-times"></i> حذف</button>
        </div>
      </div>
    ...
    <div class="section">
      ...
      <div class="section-intro">
        <p>
          نشكرك على استخدام هذا النظام الفاخر والشامل لحساب الأضرار والخسائر. يُرجى التأكد من مراجعة وتحديث البيانات وتوثيق كافة الأدلة قبل تقديم الملف. نوصي بالتواصل مع الجهات المختصة للحصول على التعويضات العادلة وفقًا للمعايير الدولية وأفضل الممارسات.
        </p>
      </div>
    </div>
  ...
  <script>
    ...
    // دالة إضافة بند جديد لكل قسم
    function addDamage(sectionId) {
      const templates = {
        directDamages: `
          <select class="damage-type">
            <option value="أضرار الأصول الثابتة">أضرار الأصول الثابتة (المباني، المعدات، الأدوات)</option>
            ...
            <option value="أضرار مؤسسية">الأضرار المؤسسية (فقدان الموظفين، إصابات العمل)</option>
          </select>
    ...
    // وظائف الطباعة والتصدير والمشاركة والإرسال عبر البريد الإلكتروني
